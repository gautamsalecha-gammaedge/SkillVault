...
(function () {
  ...
  (async function init() {
    ...
    machineSelect.innerHTML = machines.map((m) => `<option value="${escapeHtml(m)}">${escapeHtml(m)}</option>`).join("");
  ...
  function updateAskEmptyTitle() {
    document.getElementById("ask-empty-title").textContent = t("askEmptyTitle", { machine: currentMachine || "" });
  }
  ...
  function appendUserBubble(text) {
    ...
    el.innerHTML = `
      <div class="chat-avatar"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 12a4 4 0 100-8 4 4 0 000 8z"/><path d="M4 21c0-4 3.6-7 8-7s8 3 8 7"/></svg></div>
  ...
  function appendTypingIndicator() {
    ...
    el.innerHTML = `
      <div class="chat-avatar"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#17130a" stroke-width="2.2"><path d="M12 2L4 6v6c0 5 3.4 8.4 8 10 4.6-1.6 8-5 8-10V6l-8-4z"/></svg></div>
  ...
  function appendAiBubble(answer, sourcesUsed) {
    ...
    el.innerHTML = `
      ...
      <div>
        ...
        <div class="chat-meta">
          <button class="btn btn--ghost" data-speak-btn data-text="${escapeHtml(answer)}">
          ...
          <span class="chat-sources">${sourcesUsed > 0 ? t("sourcesUsed", { n: sourcesUsed }) : t("sourcesNone")}</span>
        </div>
      </div>`;
    chatScroll.appendChild(el);
    chatScroll.scrollTop = chatScroll.scrollHeight;
  ...
  function appendErrorBubble(message) {
    ...
    el.className = "chat-msg from-ai";
    el.innerHTML = `
      <div class="chat-avatar"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#17130a" stroke-width="2.2"><circle cx="12" cy="12" r="9"/><path d="M12 8v4M12 16h.01"/></svg></div>
      <div class="chat-bubble" style="border-color:#de646455;color:var(--red);">${escapeHtml(message)}</div>`;
    chatScroll.appendChild(el);
    chatScroll.scrollTop = chatScroll.scrollHeight;
  }
  ...
  async function submitQuestion() {
    ...
    if (!question) return;
    if (!currentMachine) { showToast(t("selectMachineFirst"), "error"); return; }
  ...
  async function speakText(text, btnEl) {
    ...
    btnEl.disabled = true;
    btnEl.innerHTML = `<span class="spinner"></span><span>${t("playingAnswer")}</span>`;
    try {
  ...
  /* ============================================================
     ...
     ============================================================ */
/* ============================================================
   VOICE INPUT (Ask + Tip) - WORKING VERSION
   ============================================================ */

let recognition = null;
let isRecording = false;
let currentTarget = null; // "ask" or "tip"
let finalTranscript = "";
...
function initSpeechRecognition() {
  ...
  recognition.onerror = function (event) {
    ...
    if (event.error === "not-allowed") {
      ...
      // ignore - normal when user is silent
    } else {
...
function updateVoiceUI(recording) {
  ...
  if (recording) {
    ...
    if (askStatus) askStatus.innerHTML = `<span class="listening">Listening… Speak now</span>`;
    if (askStop) askStop.style.display = "inline-block";

    tipBar?.classList.add("recording");
    tipMic?.classList.add("recording");
    if (tipStatus) tipStatus.innerHTML = `<span class="listening">Listening… Speak now</span>`;
    if (tipStop) tipStop.style.display = "inline-block";
  } else {
    askBar?.classList.remove("recording");
    askMic?.classList.remove("recording");
    if (askStatus) askStatus.textContent = "Tap mic to speak your question";
    if (askStop) askStop.style.display = "none";

    tipBar?.classList.remove("recording");
    tipMic?.classList.remove("recording");
    if (tipStatus) tipStatus.textContent = "Tap mic to speak your tip";
    if (tipStop) tipStop.style.display = "none";
  }
}
...
// Ask mic
document.getElementById("ask-mic-btn")?.addEventListener("click", function () {
...
document.getElementById("ask-stop-btn")?.addEventListener("click", stopRecording);

// Tip mic
document.getElementById("tip-mic-btn")?.addEventListener("click", function () {
...
document.getElementById("tip-stop-btn")?.addEventListener("click", stopRecording);
  /* ================= SHARE A TIP ================= */
  const tipSteps = {
  ...
  function showTipStep(name) {
    Object.values(tipSteps).forEach((el) => el.classList.remove("is-active"));
  ...
  const clarifyContinueBtn = document.getElementById("clarify-continue-btn");
  ...
  const tipSpeakConfirmBtn = document.getElementById("tip-speak-confirm-btn");

  function resetTipFlow() {
    tipDraft = { originalText: "", round: 1, question: null };
    tipText.value = "";
    clarifyAnswer.value = "";
    polishedText.value = "";
    showTipStep("write");
  ...
  tipReviewBtn.addEventListener("click", async () => {
    ...
    tipReviewBtn.innerHTML = `<span class="spinner"></span><span>${t("tipReviewing")}</span>`;
    try {
      const res = await Api.checkKnowledge(text, currentMachine, 1);
      if (!res.complete) {
        tipDraft.question = res.question;
        clarifyQuestion.textContent = res.question;
        showTipStep("clarify");
      } else {
        polishedText.value = res.polished_text || text;
        showTipStep("polish");
      }
    } catch (err) {
      showToast(err.message || t("tipErrorGeneric"), "error");
    } finally {
      tipReviewBtn.disabled = false;
      tipReviewBtn.innerHTML = `<span data-i18n="tipSubmit">${t("tipSubmit")}</span>`;
    }
  });

  clarifyContinueBtn.addEventListener("click", async () => {
    ...
    clarifyContinueBtn.innerHTML = `<span class="spinner"></span><span>${t("tipReviewing")}</span>`;
    try {
      const res = await Api.checkKnowledge(combinedText, currentMachine, 2);
      polishedText.value = res.polished_text || combinedText;
      showTipStep("polish");
    } catch (err) {
      showToast(err.message || t("tipErrorGeneric"), "error");
    } finally {
      clarifyContinueBtn.disabled = false;
      clarifyContinueBtn.innerHTML = `<span data-i18n="tipAnswerSubmit">${t("tipAnswerSubmit")}</span>`;
    }
  });

  let lastConfirmation = "";
  tipSaveBtn.addEventListener("click", async () => {
    const finalText = polishedText.value.trim();
    if (!finalText) return;

    tipSaveBtn.disabled = true;
    tipSaveBtn.innerHTML = `<span class="spinner"></span><span>${t("tipSaving")}</span>`;
    try {
      const res = await Api.addKnowledge(finalText, currentMachine, getLang());
      lastConfirmation = res.spoken_confirmation || "";
      showTipStep("success");
      showToast(t("tipSuccessTitle"), "success");

      if (lastConfirmation && tipSpeakConfirmBtn) {
        tipSpeakConfirmBtn.style.display = "inline-flex";
        tipSpeakConfirmBtn.onclick = (e) => speakText(lastConfirmation, e.currentTarget);
      } else if (tipSpeakConfirmBtn) {
        tipSpeakConfirmBtn.style.display = "none";
      }
    } catch (err) {
      showToast(err.message || t("tipErrorGeneric"), "error");
    } finally {
      tipSaveBtn.disabled = false;
      tipSaveBtn.innerHTML = `<span data-i18n="tipAcceptSubmit">${t("tipAcceptSubmit")}</span>`;
    }
  });

  tipAnotherBtn.addEventListener("click", resetTipFlow);
})();